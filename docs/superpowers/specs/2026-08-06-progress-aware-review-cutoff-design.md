...
The workflow reads only a numeric completed-group count. It does not print, upload, or persist checkpoint narratives, paths, prompts, provider bodies, or source excerpts. Malformed checkpoint files remain fail-closed through the review command's existing validation. Public Report V5, operations-state V3, and Tavernary contracts do not change.
