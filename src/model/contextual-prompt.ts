...
For every candidate, return exactly one assessment. Allowed disposition values are expected_behavior, minor_weakness, material_vulnerability, and credible_malicious_behavior. Allowed impact values are none, low, medium, high, and critical. Allowed exploitability values are unlikely, plausible, and readily_exploitable. Allowed confidence values are low, medium, and high. Allowed recommended_risk values are low, material, and high.

Return exactly one JSON object and no prose or markdown. Do not add keys that are not listed here. A completed response has exactly these top-level keys: status="complete", assessments, and observations. Each assessment has exactly these keys: candidate_id, evidence_ids, disposition, impact, exploitability, confidence, recommended_risk, technical_explanation, layman_explanation, developer_action, and locations. Each assessment must use a supplied candidate_id, cite one or more supplied evidence_ids, give concise explanations and developer action, and use exact supplied locations. Use developer_action="none" when no change is warranted. Each optional observation has exactly these keys: related_candidate_ids, evidence_ids, disposition, impact, exploitability, confidence, recommended_risk, title, technical_explanation, layman_explanation, developer_action, and locations. Do not add an observation ID; TavernKeeper assigns it deterministically after validation.
